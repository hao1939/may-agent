...
describe("session-read-guard", () => {
  ...
  it("signals reading session.jsonl from history", async () => {
  ...
  it("signals reading session.jsonl from active sessions", async () => {
    const guard = createSessionReadGuard();
    const result = await guard(
      makeCtx("read", {
        path: ".state/sessions/s_1234567890_42/session.jsonl",
  ...
  it("signals with leading ./", async () => {
    const guard = createSessionReadGuard();
    const result = await guard(
      makeCtx("read", {
        path: "./.state/sessions/s_123_1/session.jsonl",
      }),
    );
    expect(result).toBeDefined();
    expect(result!.block).toBe(false);
  ...
  it("allows reading meta.json (not session.jsonl)", async () => {
    ...
    const result = await guard(
      makeCtx("read", {
        path: ".state/sessions/s_1234567890_42/meta.json",
      }),
    );
  ...
  it("signals bash cat of session.jsonl", async () => {
    const guard = createSessionReadGuard();
    const result = await guard(
      makeCtx("bash", {
        command: "cat .state/sessions/s_123_1/session.jsonl",
      }),
    );
    expect(result).toBeDefined();
    expect(result!.block).toBe(false);
  ...
  it("signals bash less of session.jsonl", async () => {
    const guard = createSessionReadGuard();
    const result = await guard(
      makeCtx("bash", {
        command: "less .state/sessions/s_123_1/session.jsonl",
      }),
    );
    expect(result).toBeDefined();
    expect(result!.block).toBe(false);
  ...
  it("signals cat even with flags", async () => {
    const guard = createSessionReadGuard();
    const result = await guard(
      makeCtx("bash", {
        command: "cat -n .state/sessions/s_123_1/session.jsonl",
      }),
    );
    expect(result).toBeDefined();
    expect(result!.block).toBe(false);
  ...
  it("signals unbounded grep of session.jsonl", async () => {
    const guard = createSessionReadGuard();
    const result = await guard(
      makeCtx("bash", {
        command: "grep 'pattern' .state/sessions/s_123_1/session.jsonl",
      }),
    );
    expect(result).toBeDefined();
    expect(result!.block).toBe(false);
  ...
  it("signals unbounded grep with wildcards", async () => {
    const guard = createSessionReadGuard();
    const result = await guard(
      makeCtx("bash", {
        command: "grep 'error' .state/sessions/*/session.jsonl",
      }),
    );
    expect(result).toBeDefined();
    expect(result!.block).toBe(false);
  ...
  it("signals unbounded rg of session.jsonl", async () => {
    const guard = createSessionReadGuard();
    const result = await guard(
      makeCtx("bash", {
        command: "rg 'pattern' .state/sessions/s_123_1/session.jsonl",
      }),
    );
    expect(result).toBeDefined();
    expect(result!.block).toBe(false);
  ...
  it("allows grep piped to head", async () => {
    ...
    const result = await guard(
      makeCtx("bash", {
        command: "grep 'pattern' .state/sessions/s_123_1/session.jsonl | head -20",
      }),
    );
    expect(result).toBeUndefined();
  });
  ...
  it("allows grep piped to tail", async () => {
    ...
    const result = await guard(
      makeCtx("bash", {
        command: "grep 'pattern' .state/sessions/s_123_1/session.jsonl | tail -5",
      }),
    );
    expect(result).toBeUndefined();
  });
  ...
  it("allows grep -c (count only)", async () => {
    ...
    const result = await guard(
      makeCtx("bash", {
        command: "grep -c 'pattern' .state/sessions/s_123_1/session.jsonl",
      }),
    );
    expect(result).toBeUndefined();
  });
  ...
  it("allows grep -l (filenames only)", async () => {
    ...
    const result = await guard(
      makeCtx("bash", {
        command: "grep -l 'pattern' .state/sessions/*/session.jsonl",
      }),
    );
    expect(result).toBeUndefined();
  });
  ...
  it("allows grep -m (max matches)", async () => {
    ...
    const result = await guard(
      makeCtx("bash", {
        command: "grep -m 5 'pattern' .state/sessions/s_123_1/session.jsonl",
      }),
    );
    expect(result).toBeUndefined();
  });
  ...
  it("allows grep --count", async () => {
    ...
    const result = await guard(
      makeCtx("bash", {
        command: "grep --count 'pattern' .state/sessions/s_123_1/session.jsonl",
      }),
    );
    expect(result).toBeUndefined();
  });
  ...
  it("allows grep --files-with-matches", async () => {
    ...
    const result = await guard(
      makeCtx("bash", {
        command: "grep --files-with-matches 'pattern' .state/sessions/*/session.jsonl",
      }),
    );
    expect(result).toBeUndefined();
  });
  ...
  it("allows cat of meta.json via bash", async () => {
    ...
    const result = await guard(
      makeCtx("bash", {
        command: "cat .state/sessions/s_123_1/meta.json",
      }),
    );
    expect(result).toBeUndefined();
  });
  ...
  it("allows grep via shell variable indirection (can't catch this)", async () => {
    ...
    const result = await guard(
      makeCtx("bash", {
        command: `for f in .state/sessions/s_1774*/session.jsonl; do grep 'error' "$f"; done`,
      }),
    );
  ...
  it("signals grep with inline glob of session.jsonl (no limiter)", async () => {
    const guard = createSessionReadGuard();
    const result = await guard(
      makeCtx("bash", {
        command: "grep 'error' .state/sessions/s_1774*/session.jsonl",
      }),
    );
    expect(result).toBeDefined();
    expect(result!.block).toBe(false);
  ...
  it("allows jq on session.jsonl (not grep/cat pattern)", async () => {
    ...
    const result = await guard(
      makeCtx("bash", {
        command: "jq '.role' .state/sessions/s_123_1/session.jsonl | head -5",
      }),
    );
  ...
  it("allows wc on session.jsonl (size check)", async () => {
    ...
    const result = await guard(
      makeCtx("bash", {
        command: "wc -l .state/sessions/s_123_1/session.jsonl",
      }),
    );
    expect(result).toBeUndefined();
  });
