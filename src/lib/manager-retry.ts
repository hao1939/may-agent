...
export function extractFinishParams(messages: any[]): { status: string; summary: string; blockers?: { reason: string; context: string }[]; deliverables?: { path: string; description: string }[]; next_steps?: string; completed_items?: string[]; new_items?: string[]; lessons?: { category: string; content: string }[]; verification_evidence?: string[] } | null {
      ...
      for (const block of msg.content) {
        if (block?.type === "toolCall" && block.name === "finish" && block.args) {
          try {
            ...
            return { status: args.status, summary: args.summary, blockers: args.blockers, deliverables: args.deliverables, next_steps: args.next_steps, completed_items: args.completed_items, new_items: args.new_items, lessons: args.lessons, verification_evidence: args.verification_evidence };
          } catch { return null; }
        }
      }
    }
