...
export function extractFinishParams(messages: any[]): { status: string; summary: string; blockers?: { reason: string; context: string }[]; deliverables?: { path: string; description: string }[]; next_steps?: string } | null {
      ...
      for (const block of msg.content) {
        if (block?.type === "toolCall" && block.name === "finish" && block.args) {
          try {
            ...
            return { status: args.status, summary: args.summary, blockers: args.blockers, deliverables: args.deliverables, next_steps: args.next_steps };
          } catch { return null; }
        }
      }
    }
