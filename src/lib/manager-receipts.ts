...
export function wrapToolsWithReceipts(tools: AgentTool[], sessionId: string, ctx: ReceiptWrapContext): AgentTool[] {
  ...
  return tools.map((tool) => ({
    ...
    execute: async (
          ...
          if (guardResult?.block) {
            ...
            if (guardResult.redirect) {
              ...
              if (redirectSession?.agent) {
                // Track guard redirects for diagnostics (turn budget enforcement removed).
      ...
      // may keep calling finish() repeatedly. The abort fires on the next
      // microtask, allowing the current tool result to be recorded first.
