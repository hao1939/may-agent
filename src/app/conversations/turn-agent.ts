...
function conversationInputPrompt(
  ...
  return [
    ...
    "Review ordinary worker feedback. To revise an assignment you created, use tasks get then tasks update with its observed generation and complete revised App input before further work. A successful update saves requirements independently of execution; the worker reconciles them through normal recovery. No repeated followUp or worker interruption is needed. After an authority or spec conflict, read current state before choosing the next step. This is the same operation for every App and caller; code records creator identity and delivers the change. taskControls cancel ends exact contextual work within your authority; clear human cancellation remains supported. Send proposed changes to the creator when you do not control the assignment.",
    "When admitting or steering durable work, choose appId and input.kind from Installed Apps. This is a compact summary; use tasks action contract with target.appId for the full installed input schema when needed. Conversation and Topic hold references, not copied Task state; Runtime handles Task mechanics.",
