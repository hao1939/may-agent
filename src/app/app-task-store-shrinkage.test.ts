    ...
    const existingTree: TaskTree = {
      ...
      receipts: {
        "receipt-only-child": {
          ...
          acceptanceBasis: { method: "agent-judgment", evidence: ["artifact:receipt.md"] },
          failureFingerprints: [],
      ...
      Array.from({ length: 10 }, (_, index) => {
        ...
        return [
          id,
          {
            ...
            acceptanceBasis: {
              method: "agent-judgment" as const,
              evidence: [`evidence-${index}`],
            },
            failureFingerprints: [],
  ...
  it("compacts child detail once its parent is represented by a terminal receipt", () => {
    ...
    tree.receipts = {
      parent: {
        ...
        acceptanceBasis: { method: "agent-judgment", evidence: ["parent-proof"] },
        failureFingerprints: [],
      ...
      child: {
        ...
        acceptanceBasis: { method: "agent-judgment", evidence: ["large child detail"] },
        failureFingerprints: [],
        completedAt: new Date().toISOString(),
      },
    };
