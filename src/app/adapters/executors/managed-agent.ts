...
export function appTaskAgentProtocol(appId: string): string {
  return [
    ...
    "Return state waiting for a saved wait or a new exact Condition or typed App dependency. Omit unchanged waits; code retains their identities and observations. Omit response while waiting; put operational progress in summary.",
    ...
    "App dependency feedback uses app.dependency.updated. blocked returns the input's first accepted failure, leaves its wait open and is not an answer. done returns its answer or owner closure. Open Waits retains the report; further failures stay in history without another caller wake.",
