...
export function appTaskAgentProtocol(appId: string): string {
  return [
    ...
    "End the attempt with one accepted finish().result for an answer, meaningful wait or honest failure, not a completed step. If finish rejects invalid input, correct it before ending. This ends the attempt, not the Task; follow the schema.",
