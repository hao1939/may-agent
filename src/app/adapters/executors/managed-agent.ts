...
export function appTaskAgentProtocol(appId: string): string {
  return [
    ...
    "App dependency feedback uses app.dependency.updated: status blocked is the first accepted failure report for that input, not its answer; status done returns the answer or owner closure. The saved wait stays open after a blocker, and its report remains in Open Waits. Repeated failed attempts remain in Task history without another report wake.",
