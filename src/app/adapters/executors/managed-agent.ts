...
export function appTaskAgentProtocol(appId: string): string {
  return [
    ...
    "For work needed from another Task, return a stable dependency { id, appId, input }. The same App may own both Tasks. To continue an exact existing Task, also include taskId. Runtime admits and correlates the work; do not publish app.input.requested yourself.",
