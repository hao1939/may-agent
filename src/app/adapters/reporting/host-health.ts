...
export const HOST_HEALTH_FAILURE_EVENTS = [
  "handler.failed", "subscriber.failed", "app.observer.failed", "agent.config_invalid",
  "session.resume_failed", "message.delivery_failed", "metric.measurement.failed",
] as const;

function executionHealth(db: SqliteDb, table: "sessions" | "workflow_runs", start: number, end: number): HostExecutionHealth {
  const id = table === "sessions" ? "sessionId" : "runId";
  const known = "'done', 'error', 'blocked', 'interrupted'";
  const counts = db.prepare(`SELECT COUNT(*) AS ended,
    ...
    FROM ${table} WHERE endedAt >= ? AND endedAt < ?`).get(start, end) as
      Pick<HostExecutionHealth, "ended" | "done" | "error" | "blocked" | "interrupted" | "other">;
  const running = db.prepare(`SELECT COUNT(*) AS count FROM ${table} WHERE status = 'running'`).get()!.count as number;
  const undated = db.prepare(`SELECT COUNT(*) AS count FROM ${table}
    WHERE endedAt IS NULL AND status IN (${known}) AND startedAt >= ? AND startedAt < ?`).get(start, end)!.count as number;
  const recentErrors = db.prepare(`SELECT ${id} AS executionId, endedAt FROM ${table}
    WHERE status = 'error' AND endedAt >= ? AND endedAt < ?
    ORDER BY endedAt DESC, ${id} DESC LIMIT ?`).all(start, end, HOST_HEALTH_DETAIL_LIMIT) as HostExecutionHealth["recentErrors"];
...
export function readHostHealth(
  ...
  try {
    const byType = db.prepare(`SELECT event_type AS type, COUNT(*) AS count FROM events
      WHERE event_type IN (${types}) AND timestamp >= ? AND timestamp < ?
      GROUP BY event_type ORDER BY event_type`).all(...HOST_HEALTH_FAILURE_EVENTS, start, now) as HostHealthSnapshot["runtimeFailures"]["byType"];
    const recent = db.prepare(`SELECT id AS eventId, event_type AS type, timestamp FROM events
      WHERE event_type IN (${types}) AND timestamp >= ? AND timestamp < ?
      ORDER BY timestamp DESC, id DESC LIMIT ?`).all(...HOST_HEALTH_FAILURE_EVENTS, start, now, HOST_HEALTH_DETAIL_LIMIT) as HostHealthSnapshot["runtimeFailures"]["recent"];
    const total = byType.reduce((sum, row) => sum + row.count, 0);
    const snapshot: HostHealthSnapshot = {
      generatedAt: now, window: { start, end: now },
      coverage: { retainedOnly: true, executionScope: "all" },
      executions: { agents: executionHealth(db, "sessions", start, now), workflows: executionHealth(db, "workflow_runs", start, now) },
