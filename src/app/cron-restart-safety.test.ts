...
import { Cron, computeCronResumeDelay } from "./cron.ts";
...
function cronAt(projectRoot: string, entryName: string, intervalMs: number, handler: () => Promise<void>): Cron {
  const cron = new Cron(join(projectRoot, "missing-cron.json"), {} as any, () => "test", undefined, projectRoot);
...
describe("Cron restart-safe schedule cadence", () => {
  it("reconstructs a pre-due registration without an early or duplicate emission", async () => {
    ...
    recordStarted(projectRoot, "restart-before-due", startedAt);
    let emissions = 0;
  ...
  it("preserves first-registration offset and bounded jitter for hourly and daily schedules", () => {
    expect(computeCronResumeDelay({ intervalMs: 3_600_000, lastFireTime: null, now: 0, offsetMs: 42_000 })).toBe(
      42_000,
    );
    expect(computeCronResumeDelay({ intervalMs: 3_600_000, lastFireTime: null, now: 0, random: () => 0.5 })).toBe(
      150_000,
    );
    expect(computeCronResumeDelay({ intervalMs: 86_400_000, lastFireTime: null, now: 0, random: () => 0.5 })).toBe(
      150_000,
    );
  });
  ...
  it("produces a deterministic may-agent platform-review restart trace with no tick before 21,600,000 ms", () => {
    ...
    const remaining = computeCronResumeDelay({
      intervalMs,
