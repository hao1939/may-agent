...
describe("canonical App task capability boundary", () => {
  it("keeps App task-runtime imports behind the host capability boundary", () => {
    const runtime = readFileSync(join(import.meta.dir, "app-runtime.ts"), "utf8");
    expect(runtime).not.toContain("app-task-runtime");
    ...
    expect(socket).not.toContain("app-task-runtime");
  });
});
