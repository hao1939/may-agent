...
describe("model registry", () => {
  it("routes models through the configured endpoint with canonical model names", () => {
    ...
    expect(registry["gpt-5.4"]?.baseUrl).toBe("http://model-endpoint:4000");
    expect(registry["gpt-5.4"]?.apiKey).toBe("endpoint-key");
    expect(registry["claude-opus-4-6"]?.id).toBe("claude-opus-4-6");
    expect(registry["claude-opus-4-6"]?.contextWindow).toBe(72_000);
    expect(registry["gpt-5.2"]?.contextWindow).toBe(400_000);
  ...
  it("uses local no-auth endpoint defaults", () => {
    ...
    expect(registry["gpt-5.4"]?.baseUrl).toBe("http://localhost:4000");
    expect(registry["claude-opus-4-6"]?.apiKey).toBe("not-needed");
  });
});
