...
test("ordinary observer reads Host facts and its App owns the resulting work", async () => {
  const sb = await buildSandbox({
    fixtureAgents: ["may"],
    fixtureProjects: ["sample-health.app"],
    fixtureWorkflows: { may: ["host-health-proof"] },
    cronJson: { may: [] },
    daemonArgs: ["--socket"],
  });
  try {
    ...
    try {
      ...
      const event = db
        .query("SELECT data FROM events WHERE event_type = 'sample.health.observed' ORDER BY id DESC LIMIT 1")
        .get() as { data: string };
    ...
    try {
      evidence = {
        ...
        failures: db
          .query(
            "SELECT event_type, data FROM events WHERE event_type IN ('app.observer.failed', 'subscriber.failed') LIMIT 3",
          )
          .all(),
      };
    } finally {
