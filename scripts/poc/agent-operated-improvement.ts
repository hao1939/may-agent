  ...
  try {
    ...
    if (!live) {
      ...
      const tryTarget: AgentTool = {
        ...
        description: `Ask May an ordinary request using the currently ACTIVE definition, not unactivated source edits. The target can read only its active definition snapshot; your evidence directory is not accessible to it. Returns actual revision, answer, tool-error count and a detailed evidence reference. Read-only target execution; at most ${maxProbes} trials. Use results to judge your change; no hidden expected answers are supplied.`,
